...
export default function Categories() {
  const { getCard, getWishList, setIsCardCounter, setIsWishList,queryData,setSearchResult,searchResult } =
...
useEffect(() => {
    ...
    const options = {
      ...
      let searchData = categories

      useEffect(() => {
        if (searchData) {
          if (queryData) {
            const fuse = new Fuse(searchData, options);
            let result = fuse.search(queryData).map((res) => {
            return res.item;
            });
...
h78.747C231.693,100.736,232.77,106.162,232.77,111.694z"
         />
          ...
          <>
            <div className="container mx-auto gap-10 pt-10 grid py-10 lg:grid-cols-6 md:grid-cols-4 sm:grid-cols-2">
              {searchResult?.map((category) => (
                <div key={category.id} className="shadow-xl">
                  <img
